...
(function($) {
	...
	Calendar.prototype = {
		...
		_renderSemiHeader: function() {
			var semiHeader = $(document.createElement('div'));
			var semiHeaderTable = $(document.createElement('table'));			
			var prevMonth = $(document.createElement('th'));
			var prevIcon = $(document.createElement('i'));
			var currentMonth = $(document.createElement('th'));
			var nextMonth = $(document.createElement('th'));
			var nextIcon = $(document.createElement('i'));
			var semiCurrentMonth = dates[this.options.language].months[this.options.startMonth];
			var semiCurrentYear = this.options.startYear;

			semiHeader.addClass('calendar-semi-header');
			prevIcon.addClass('glyphicon glyphicon-chevron-left');
			prevMonth.addClass('prev-month');
			prevMonth.append(prevIcon);
			currentMonth.text(semiCurrentMonth + " " + semiCurrentYear);
			nextIcon.addClass('glyphicon glyphicon-chevron-right');
			nextMonth.addClass('next-month');
			nextMonth.append(nextIcon);
			semiHeaderTable.append(prevMonth).append(currentMonth).append(nextMonth);
			semiHeader.append(semiHeaderTable);

			this.element.append(semiHeader);
		},
		...
		_renderBody: function() {
			...
			for(var m = this.options.startMonth; m < this.options.endMonth; m++) {
				...
				titleCell.text( dates[this.options.language].months[month] + ' ' + ( m < 12 ? this.options.startYear : this.options.startYear + 1 ) );
		...
		_applyEvents: function () {
			...
			this.element.find('.calendar-semi-header .prev-month').click(function() {
				if(!$(this).hasClass('disabled')) {
					_this.element.find('.months-container').animate({'margin-left':'100%'},100, function() {
						_this.element.find('.months-container').hide();
						_this.element.find('.months-container').css('margin-left', '0');
			...
			this.element.find('.calendar-semi-header .next-month').click(function() {
				if(!$(this).hasClass('disabled')) {
					_this.element.find('.months-container').animate({'margin-left':'-100%'},100, function() {
						_this.element.find('.months-container').hide();
						_this.element.find('.months-container').css('margin-left', '0');
		...
		setMonth: function(month) {
			...
			var currentYear = this.options.startYear;
			if(!isNaN(parsedMonth)) {
				if (parsedMonth === -1) {
					this.options.startMonth = 11;
					this.options.endMonth = 12;
					this.options.startYear = currentYear - 1;
				} else if (parsedMonth === 12) {
					this.options.startMonth = 0;
					this.options.endMonth = 1;
					this.options.startYear = currentYear + 1;
				} else {
					this.options.startMonth = parsedMonth;
					this.options.endMonth = parsedMonth + 1;
				}
				this._render();
			}
		},
